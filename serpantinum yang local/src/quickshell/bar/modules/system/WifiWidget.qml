...
Rectangle {
    ...
    property string nmcliEthernetDevice: ""
    property bool ethernetConnected: ethStatus === "Connected" || nmcliEthernetConnected
    property bool showEthernet: ethernetConnected || (isDesktop && !isWifiOn)
    ...
    function isEthernetDevice(device) {
        if (!device) return false;
        try {
            if (device.type === DeviceType.Wired) return true;
        } catch (e) {}

        // WiredDevice exposes these two properties. This fallback keeps the
        // widget working across Quickshell networking enum/API revisions.
        return device.hasLink !== undefined && device.linkSpeed !== undefined;
    }

    function isWifiDevice(device) {
        if (!device) return false;
        try {
            if (device.type === DeviceType.Wifi) return true;
        } catch (e) {}
        return device.networks !== undefined && device.scannerEnabled !== undefined;
    }

    Process {
        id: ethernetProbe
        running: wifiWidgetRoot.moduleActive
        command: ["bash", "-c", "LC_ALL=C nmcli -t -f DEVICE,TYPE,STATE device status 2>/dev/null | grep ':ethernet:' | head -n 1"]
        stdout: StdioCollector {
            onStreamFinished: {
                let line = this.text.trim();
                let parts = line === "" ? [] : line.split(":");
                wifiWidgetRoot.nmcliEthernetDevice = parts.length >= 1 ? parts[0] : "";
                wifiWidgetRoot.nmcliEthernetConnected = parts.length >= 3 && parts[2] === "connected";
    ...
    Timer {
        ...
        running: wifiWidgetRoot.moduleActive
        onTriggered: {
            if (!ethernetProbe.running) ethernetProbe.running = true;
        }
    }
    ...
    Item {
        ...
        Repeater {
            ...
            Item {
                ...
                function syncDevice() {
                    if (!device) return;
                    if (wifiWidgetRoot.isEthernetDevice(device)) {
                        wifiWidgetRoot.ethDevice = device;
                    } else if (wifiWidgetRoot.isWifiDevice(device)) {
                ...
                Connections {
                    target: (device && wifiWidgetRoot.isEthernetDevice(device)) ? device : null
                    ignoreUnknownSignals: true
    ...
    function updateNetworkData() {
        ...
        } else if (nmcliEthernetConnected) {
            ethStatus = "Connected";
        } else {
            ethStatus = "Ethernet";
        }
    ...
    Row {
        ...
        ClickButton {
            ...
            onClicked: {
                let targetMode = showEthernet ? "eth" : "wifi";
                Quickshell.execDetached(["bash", "-c", Caching.serpantinumDir + "/scripts/qs_manager.sh toggle network " + targetMode]);
            }
        }
    }
}
